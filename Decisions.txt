...
4.Надо ставить на клик вызов AJAX и вставлять поле с типом HIDDEn для того чтобы ловить переменную isset...
--------------------------------------------------
...
* !!!!!!!!!!!! -> .HTACCESS (ЧПУ) <- !!!!!!!!!!  (структура папок: nameproject/config,library....., index.php, .htaccess)
      ...
      Для того чтобы работал ЧПУ на сайте , надо:
	     1. пути к ссылкам переделать 
		       было :   <a href="/category/1"> 
			   стало:   <a href="{$SiteWebPath}/category/1">   // $SiteWebPath = "http://localhost/bigp/shop";
		 2. пути к стилям переделать
                было:   <link rel="stylesheet" href="{$TemplateWebPath}css/booter/css/bootstrap.css" >   // $SiteWebPath = "templates/{$template}/";
                стало:	<link rel="stylesheet" href="{$TemplateWebPath}css/booter/css/bootstrap.css" >	 // $SiteWebPath = "http://localhost/bigp/shop/templates/{$template}/";	
		3. При переносе на сервер удостоверится в правильности путей к стилям:
                !!! например:  файл называеться "w3.css", а в путях(src) - "W3.css", 
				               на системах Windows будет работать , 
							   но не на сервере где система Unix		
...
-------------------------------------------------- 
// Процедура вставки с параметрами 1 - 4 ШТ. ###
...
--------------------------------------------------
// Связь между двумя таблами на удаление
